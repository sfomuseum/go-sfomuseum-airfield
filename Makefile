...
cli-create:
	go build -mod $(GOMOD) -ldflags="$(LDFLAGS)" -o bin/create-airline cmd/create-airline/main.go

cli-lookup:
	go build -mod $(GOMOD) -ldflags="$(LDFLAGS)" -o bin/lookup cmd/lookup/main.go

cli-stats:
	go build -mod $(GOMOD) -ldflags="$(LDFLAGS)" -o bin/tailnumbers cmd/tailnumbers/main.go
...
local-scan:
	/usr/local/bin/sonar-scanner/bin/sonar-scanner \
		-Dsonar.projectKey=go-sfomuseum-airfield \
		-Dsonar.sources=. \
