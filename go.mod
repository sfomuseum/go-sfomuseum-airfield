module github.com/sfomuseum/go-sfomuseum-airfield

go 1.16

require (
	github.com/aaronland/go-roster v0.0.2
	github.com/mitchellh/go-wordwrap v1.0.1
	github.com/sfomuseum/go-sfomuseum-reader v0.0.1
	github.com/sfomuseum/go-sfomuseum-writer v0.2.4
	github.com/tidwall/gjson v1.9.2
	github.com/whosonfirst/go-ioutil v1.0.0
	github.com/whosonfirst/go-reader v0.9.0
	github.com/whosonfirst/go-reader-http v0.2.2
	github.com/whosonfirst/go-reader-whosonfirst-data v1.2.2
	github.com/whosonfirst/go-whosonfirst-export/v2 v2.3.5
	github.com/whosonfirst/go-whosonfirst-feature v0.0.9
	github.com/whosonfirst/go-whosonfirst-fetch v0.1.0
	github.com/whosonfirst/go-whosonfirst-iterate/v2 v2.0.0
	github.com/whosonfirst/go-whosonfirst-uri v1.1.0
	github.com/whosonfirst/go-writer v0.6.0
)
